  ...
  return (
    ...
    >
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[70vh] overflow-y-auto shadow-2xl">
        <div
          className={`sticky top-0 bg-gradient-to-r ${plan.gradient} p-3 rounded-t-lg relative`}
        >
        ...
        <div className="p-3 text-xs space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M3 5a2 2 0 012-2h3.28a1 1 0 01.848.536l1.42 2.028a1 1 0 01-.024.912l-1.264 6.324a1 1 0 01-1.028.868 1 1 0 01-1.028-.868L3.3 12.028A1 1 0 012 11.118V5a2 2 0 012-2z"
                />
              </svg>
              <span className="font-bold text-rose-700">Llamadas Ilimitadas</span>
            </div>
            <div className="ml-6 mt-1 text-slate-700">
              A nivel Nacional + 500 SMS
            </div>
          </div>
          <div className="bg-blue-50 rounded-lg p-2 border border-blue-200">
            <div className="flex items-center gap-2">
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M9 7H6a2 2 0 00-2 2v9a2 2 0 002 2h12a2 2 0 002-2V9a2 2 0 00-2-2h-3M9 7V4M9 4H6M9 4h12M9 4v3M15 7h3"
                />
              </svg>
            ...
            {plan.nombre.includes("Ilimitado") && (
              <div className="ml-6 mt-1 text-slate-700">
                {plan.precio === "S/55.9"
                  ? "66 GB en alta velocidad"
                  : plan.precio === "S/65.9"
                  ? "80 GB en alta velocidad"
                  : plan.precio === "S/74.9"
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M21 12a9 9 0 00-9-9v9h9z"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M15 10a4 4 0 00-8 0v6a4 4 0 008 0v-6z"
                />
              </svg>
              <span className="text-slate-700">
                {plan.precio === "S/20.9"
                  ? "+ 200 min LDI EE.UU./Canadá"
                  : plan.precio === "S/25.9"
                  ? "+ 250 min LDI EE.UU./Canadá"
                  : plan.precio === "S/35.9"
                  ? "+ 300 min LDI EE.UU./Canadá"
                  : plan.precio === "S/45.9"
                  ? "+ 350 min LDI EE.UU./Canadá"
            ...
            <div className="bg-slate-50 rounded-lg p-2 border border-slate-200">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-slate-800">
                  Apps ilimitadas (12 meses)
                </span>
              </div>
              <div className="mt-1 flex flex-wrap gap-1">
                {["WhatsApp", "Facebook Fotos", "Messenger", "Instagram", "Waze"].map(
                  (app) => (
                    <span
                      key={app}
                      className="px-1.5 py-0.5 bg-white text-[10px] rounded border border-slate-300 shadow-sm"
                    >
                      {app}
                    </span>
                  )
          ...
          <div className="bg-green-50 rounded-lg p-2 border border-green-200">
            <div className="text-slate-700">
              ...
              {plan.precio === "S/20.9"
                ? "50 MB promocionales (12 meses) para usarlos como datos internacionales en determinados países de América y Europa."
                : plan.precio === "S/25.9"
                ? "50 MB promocionales (12 meses) para usarlos como datos internacionales en determinados países de América y Europa, así como WhatsApp de texto ilimitado."
                : plan.precio === "S/35.9"
                ? "250 MB promocionales (12 meses) para usarlos como datos internacionales en determinados países de América y Europa, así como WhatsApp de texto ilimitado."
                : plan.precio === "S/45.9"
                ? "1.25 GB promocionales (12 meses) para usarlos como datos internacionales en determinados países de América y Europa, así como WhatsApp de texto ilimitado."
                : plan.precio === "S/55.9" || plan.precio === "S/65.9"
                ? "2 GB promocionales (12 meses) para usarlos como datos internacionales en determinados países de América y Europa, así como WhatsApp de texto ilimitado."
                : plan.precio === "S/74.9" || plan.precio === "S/85.9"
                ? "3 GB promocionales (12 meses) para usarlos como datos internacionales en determinados países de América y Europa, así como WhatsApp de texto ilimitado."
...
export default function App() {
        ...
        const combinados = equiposLista.map((eq) => {
          ...
          if (!acc) {
            ...
            for (const a of accesoriosLista) {
              ...
              if (modeloAcc && modeloEq.includes(modeloAcc)) {
                acc = a;
                break;
              }
            }
  ...
  useEffect(() => {
    const sedes = Array.from(
      new Set(resultados.map((r) => r.hoja).filter(Boolean))
    );
  ...
  const resultadosFiltrados = resultados
    ...
    .sort((a, b) =>
      sortStockDesc
        ? (b.stock_final || 0) - (a.stock_final || 0)
        : (a.stock_final || 0) - (b.stock_final || 0)
    );
  ...
  const renderContrato = () => {
    const planesArray = Object.entries(planesData);
    const groupSize = 3;
    const groups = [];
    for (let i = 0; i < planesArray.length; i += groupSize) {
      groups.push(planesArray.slice(i, i + groupSize));
    }

    const handlePrev = () => {
      setCurrentIndex((prev) =>
        prev > 0 ? prev - 1 : groups.length - 1
      );
    };

    const handleNext = () => {
      setCurrentIndex((prev) =>
        prev < groups.length - 1 ? prev + 1 : 0
      );
    };
              ...
              <div className="p-8 sm:p-10 space-y-8">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-slate-700 leading-relaxed">
                        ...
                        , vamos a iniciar con la grabación del contrato.
                      </p>
                      <p className="text-slate-700 mt-3 leading-relaxed">
                        Siendo hoy{" "}
                        <span className="font-semibold">(día, mes, año)</span>,
                        para continuar con la renovación del número{" "}
                        <span className="font-semibold text-indigo-600">
                          XXX
                        </span>
                        , por su seguridad validaremos los siguientes datos, me
                        indica:
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-2xl p-6 border-2 border-indigo-100 shadow-lg animate-slide-in-right" style={{ animationDelay: '0.2s' }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      "Número de DNI",
                      "Correo electrónico",
                      "Número adicional de referencia",
                      "Dirección de entrega",
                    ].map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-3 bg-gradient-to-r from-slate-50 to-indigo-50 p-3 rounded-xl border border-slate-200 hover:shadow-md transition-all duration-300 hover:scale-105"
                      >
                ...
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-l-4 border-amber-400 rounded-xl p-5 shadow-lg animate-slide-in-right" style={{ animationDelay: '0.3s' }}>
                  <p className="font-bold text-amber-900 mb-2">
                    ⚠️ Dirección Completa Requerida
                  </p>
                  <ul className="space-y-1 text-sm text-amber-800">
                    <li className="flex items-start gap-2">
                      <span className="text-amber-500 mt-0.5">•</span> Calle,
                      número de puerta, distrito y referencias
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-amber-500 mt-0.5">•</span> Manzana,
                      lote, urbanización, distrito y referencias
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-6 border border-blue-200 shadow-lg animate-slide-in-right" style={{ animationDelay: '0.4s' }}>
                  ...
                  <p className="text-slate-700 mb-4 leading-relaxed">
                    Sr/Sra.{" "}
                    <span className="font-semibold text-blue-600">XXX</span>{" "}
                    ahora pasará a tener el plan{" "}
                    <span className="font-semibold text-blue-600">XXX</span> con
                  ...
                  <p className="text-slate-600 text-sm mb-5">
                    Con este plan, obtendrá los siguientes beneficios:
                  </p>
                  ...
                  <div className="relative">
                            ...
                            {group.map(([key, plan]) => (
                              ...
                              >
                                <button
                                  ...
                                  className={`group relative overflow-hidden text-white rounded-lg p-3 h-16 w-full shadow transition-all duration-200 bg-gradient-to-r ${plan.gradient} hover:scale-105`}
                                >
                    ...
                    <button
                      onClick={handlePrev}
                      className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 w-6 h-6 rounded-full bg-white shadow-md flex items-center justify-center hover:scale-110 transition-all"
                    ...
                    <button
                      onClick={handleNext}
                      className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 w-6 h-6 rounded-full bg-white shadow-md flex items-center justify-center hover:scale-110 transition-all"
                  ...
                  <div className="flex justify-center gap-1.5 mt-3">
                    {groups.map((_, idx) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all ${currentIndex === idx ? 'w-6 bg-blue-600' : 'w-1.5 bg-blue-300'}`}
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border border-emerald-200 shadow-lg animate-slide-in-right" style={{ animationDelay: '0.6s' }}>
                  ...
                  <div className="space-y-3 text-sm text-slate-700">
                    <p>
                      Para finalizar la renovación, le detallo lo siguiente:
                    </p>
                    <ul className="space-y-2 ml-4">
                      <li className="flex items-start gap-2">
                        <span className="text-emerald-500 font-bold">•</span> Estamos procediendo a registrar la solicitud del equipo{" "}
                        <span className="font-semibold text-emerald-700">
                          XXX
                        </span>{" "}
                        (marca, modelo, capacidad, color)
                      </li>
                      <li className="flex items-start gap-2">
                        <span className="text-emerald-500 font-bold">•</span> Cuota inicial de{" "}
                      ...
                      <li className="flex items-start gap-2">
                        <span className="text-emerald-500 font-bold">•</span> El equipo adquirido tiene un contrato de permanencia de{" "}
                        <span className="font-semibold">12 meses</span>
                      </li>
                    </ul>
                    <div className="bg-white/50 rounded-xl p-4 mt-4 border border-emerald-200">
                      <p className="text-xs leading-relaxed">
                        En caso realice la baja del servicio móvil, migra a
                        prepago o realiza un cambio de plan a uno menor,
                        Telefónica podrá resolver el financiamiento y cobrar
                        todas las cuotas. Es obligación del cliente pagar la
                        totalidad de las cuotas. Recuerde que en caso de no
                        pagar una o mas cuotas del equipo o de la totalidad del
                        precio del equipo, en caso de resolverse el financiamiento,
                        Movistar podrá optar por bloquear el equipo de manera
                        remota y reportarlo en las centrales de riesgo.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-violet-50 to-purple-50 rounded-2xl p-6 border border-violet-200 shadow-lg animate-slide-in-right" style={{ animationDelay: '0.7s' }}>
                  ...
                  <div className="space-y-3 text-sm text-slate-700">
                    ...
                    <ul className="space-y-2 ml-4">
                      <li className="flex items-start gap-2">
                        <span className="text-violet-500 font-bold">•</span> Estamos procediendo a registrar la solicitud del equipo{" "}
                        <span className="font-semibold text-violet-700">
                          XXX
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <span className="text-violet-500 font-bold">•</span> Pago único de{" "}
                        <span className="font-bold text-violet-700">
                          S/ XXX
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <span className="text-violet-500 font-bold">•</span> El equipo adquirido tiene un contrato de permanencia de{" "}
                        <span className="font-semibold">12 meses</span>
                      </li>
                    </ul>
                    <div className="bg-white/50 rounded-xl p-4 mt-4 border border-violet-200">
                      <p className="text-xs leading-relaxed">
                        Nuestro delivery le efectuara el cobro correspondiente del
                        equipo. Cabe recalcar que nuestro delivery no acepta
                        efectivo por lo que el pago deberá efectuarse con tarjeta
                        de débito o crédito Visa, MasterCard y Diners.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-pink-50 to-rose-50 rounded-2xl p-6 border border-pink-200 shadow-lg animate-slide-in-right" style={{ animationDelay: '0.8s' }}>
                  ...
                  <p className="text-sm leading-relaxed">
                    A fin de crear ofertas personalizadas y recibir anuncios
                    comerciales, autoriza a Movistar a hacer uso y tratamiento de
                    sus datos personales. Te agradeceré decir{" "}
                    <span className="font-bold text-pink-700">SÍ ACEPTO</span>.
                  </p>
                  <div className="bg-white/50 rounded-xl p-4 mt-4 border border-red-200">
                    <p className="text-xs leading-relaxed">
                      Movistar resguardara tus datos personales según la
                      legislación vigente. Para más información, consulta la
                ...
                <div className="bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 rounded-2xl p-6 border-2 border-indigo-300 shadow-xl animate-slide-in-right" style={{ animationDelay: '0.9s' }}>
                  <div className="text-center">
                    ...
                    <p className="text-slate-800 text-lg leading-relaxed font-medium">
                      Habiendo sido informado de las características del contrato,
                      le agradeceré decir{" "}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
              ...
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl shadow-sm border border-purple-200 p-4">
                ...
                <p className="text-lg font-bold text-slate-800">
                  {resultadosFiltrados.length > 0
                    ? Math.round(
                        (itemsActivos / resultadosFiltrados.length) * 100
                      )
                    : 0}
                  %
                </p>
                  ...
                  {resultadosFiltrados.map((r) => (
                    ...
                    >
                      <td className="px-4 py-2">
                        <span className="font-mono text-sm">{r.codigo_sap}</span>
                      </td>
      ...
      {selectedCodigoSap && (
        ...
        >
          <div
            className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[80vh]"
